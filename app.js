const {prefix, name, avatar, token} = require("./config.json");
...
client.on("ready", () => {
  ...
  client.user.setAvatar(avatar).catch(err => {
    console.error(err);
  });
...
client.on("message", (message) => {
  //console.log(`Got message: ${message.content} {${message.author.username}}`);
  ...
  console.log(`Got command "${commandName}" with args "${args.join(", ")}" from: ${message.author.username}`);
